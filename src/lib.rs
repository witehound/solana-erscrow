...
declare_id!("6PjGQ8Y7wXeVqqeBgXm81iWsTbDsJPZYqQcDYSGbHPth");
...
mod hello_anchor {
    ...
    pub fn accept_deal(ctx: Context<EscrowParty>) -> Result<()> {
        ...
        verify_unique_address(bk, s.key())?;

        es.seller = Some(s.key());

        verify_buyer_seller(es.buyer, es.seller)?;
    ...
    pub fn withdraw_fund(ctx: Context<EscrowParty>) -> Result<()> {
        ...
        let buyerkey = es.buyer.unwrap();

        verify_owner(buyerkey, s.key())?;
